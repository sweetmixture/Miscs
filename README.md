...
ACCESS_KEY="${1:-}"
SECRET_KEY="${2:-}"
ENDPOINT="${3:-https://s3.amazonaws.com}"
ALIAS="${4:-s3}"
...
[[ -f "$CONFIG_FILE" ]] && cp -p "$CONFIG_FILE" "$CONFIG_FILE.bak"
BASE=$( [[ -f "$CONFIG_FILE" ]] && cat "$CONFIG_FILE" || echo '{"version":"10","aliases":{}}' )

jq -n --argjson base "$BASE" \
      --arg a "$ALIAS" --arg u "$ENDPOINT" \
      --arg ak "$ACCESS_KEY" --arg sk "$SECRET_KEY" '
  ($base | .version = "10" | .aliases //= {})
  | .aliases[$a] = {url:$u, accessKey:$ak, secretKey:$sk, api:"S3v4", path:"auto"}
' > "$CONFIG_FILE"
